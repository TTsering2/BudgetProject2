export const NewIncomeForm = () =>{
    ...
    return(
            ...
            <input type="text" placeholder="Title" required className="block w-5/12 m-auto mt-5 rounded px-5 py-1" />
            <input type="text" placeholder="Amount" required className="block w-5/12 m-auto mt-5 rounded px-5 py-1" />
             <select id="category" name="category" required className="block w-5/12 m-auto px-3 mt-5 py-1 border rounded-md focus:outline-none focus:border-blue-500">
                    {types.map((element, key) => <option className=" w-200 " key={key}>{element}</option>)}
            </select>
