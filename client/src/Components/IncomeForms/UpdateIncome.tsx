...
export const UpdateIncomeForm = ({
  ...
  const DeleteIncomeEntry = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5112/api/Income/incomeId=${initialData.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const EditSelectedEntry = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5112/api/Income/${IncomeEntry.id}`,
        {
          method: "PATCH",
          headers: {
  ...
  return (
    displayForm && (
      <form
        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-4/12 bg-[#EEEEEE] p-8 rounded shadow-md"
        onSubmit={EditSelectedEntry}
      >
        {notification != "" ? (
          <p className="text-black text-center text-[#11D3B0]">
            {notification}
          </p>
        ) : (
          ""
        ...
        <button
          className="absolute top-0 right-0 bg-[#DD3535] text-white py-2 px-4 rounded my-2 mx-2 text-center mx-auto block"
          onClick={() => setDisplay(false)}
        >
          X
        </button>
        <h1 className="text-center font-semibold text-size-18">
        ...
        <div className="flex flex-row gap-5">
          ...
          />
          <button
            className="block mt-5 bg-[#E14942] text-white cursor-pointer m-auto hover:bg-[#CE261E] font-semibold py-1 px-8 margin-auto rounded-md focus:outline-none"
