...
const ExpensePage = () => {
    ...
    //Get user expense
     const getAllUserExpense = async() => {
        try{
            ...
            const response = await fetch('http://localhost:5112/api/Expense/userId=3');
            if(!response.ok){
    ...
    const expenseByType: ExpenseByType = userData.reduce((dataContainer:ExpenseByType, element:UserData) => {
                ...
                <section className="h-5/6">
                    <div className="bg-primary-white font-bold w-[1350px] m-auto p-6 rounded mt-6">
                        ...
                        <div className="w-[1350px] m-auto my-5 " >
                            <button className="bg-primary-green-blue text-white p-2 px-7 rounded  text-center mx-auto block">Add a New Expense</button>
                        </div>
                   
                    <div className="w-[1350px] m-auto">
                    {Object.entries(expenseByType).map(([type, {data, totalExpense}], key) => (
                        <div key={key} className="p-2 bg-primary-white w-[1350px] m-auto   rounded-xl mb-9 ">
                            <div className="w-[1350px] m-auto shadow-gray-100 py-2  ">
                                <div className="w-[1250px] m-auto border-b-2 border-primary-green-blue pb-3 flex flex-row gap-2  items-center">
                                    ...
                                    <h3 className="text-2xl font-semibold">${totalExpense}</h3>
                                </div>
